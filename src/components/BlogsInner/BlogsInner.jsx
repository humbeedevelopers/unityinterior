...
const BlogsInner = ({ blog }) => {
 ...
 const getFullText = () => {
  ...
  // Add intro
  if (blog?.intro) {
    finalText += blog.intro + ". ";
  }

  if (contentRef.current) {
    // Clone DOM so we don't modify real content
    const cloned = contentRef.current.cloneNode(true);

    // ❌ Remove unwanted elements
    cloned.querySelectorAll("img").forEach(el => el.remove());
    cloned.querySelectorAll("figure").forEach(el => el.remove());
    cloned.querySelectorAll("figcaption").forEach(el => el.remove());
    cloned.querySelectorAll("svg").forEach(el => el.remove());
    cloned.querySelectorAll("button").forEach(el => el.remove());

    // Extract clean text
    let extractedText = cloned.innerText;

    // Improve speech flow (add pauses)
    extractedText = extractedText.replace(/\n/g, ". ");

    finalText += extractedText;
  }

  fullTextRef.current = finalText.trim();
  ...
  //   fullTextRef.current = fullText.trim();
  //   return fullTextRef.current;
  // };
        ...
        {/* {blog?.sections?.map((section, index) => (
            <div className="blogsInner__section" key={index}>
              ...
              {section.paragraphs.map((para, i) => {
                ...
                return (
                  <div className="blogsInner__paragraph" key={i}>
                    {para.bold && (
