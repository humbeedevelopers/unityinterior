...
async function getServicePageData() {
    const res = await fetch(
        "https://websiteadmin.unityinteriors.com/wp-json/wp/v2/service-pages?slug=architectural-planning-service&acf_format=standard",
        { next: { revalidate: 60 } }
    );
...
async function getRelatedProjects() {
    const res = await fetch(
        "https://websiteadmin.unityinteriors.com/wp-json/wp/v2/projects?project_category_slug=architectural-planning&acf_format=standard&per_page=100",
        { next: { revalidate: 60 } }
    );
    if (!res.ok) return [];
    const data = await res.json();
...
export default async function ArchitecturalPlanning() {
    ...
    return (
        <div>
            <HeroService
                title={acf.service_name || "Architectural Planning"}
